...
# Constants
BUCKET_POSTS_NAME = "influencer-posts"
MAX_ITEMS = 10
...
def scrape_posts_from_urls(urls: List[Dict[str, str]], batch_size: int = 2) -> List[Dict]:
    '''
...
def upload_json_to_gcs(data: List[Dict]) -> str:
    """
        ...
        params:
            data (List[Dict]): list of scraped posts
    ...
    bucket = client.bucket(BUCKET_POSTS_NAME) 
    ...
    destination_blob_path = f"facebook/year={year}/month={month}/day={day}/facebook_posts_{timestamp}.json"
    ...
    print(f"Upload JSON data to gs://{BUCKET_POSTS_NAME}/{destination_blob_path} completely.")

    return f"gs://{BUCKET_POSTS_NAME}/{destination_blob_path}"